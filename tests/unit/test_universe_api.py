...
def test_universe_api_get_constituents():
    ...
    sec1 = Security(security_uuid="s1", current_symbol="TCS", name="Tata Consultancy Services", listing_date=date(2004, 1, 1))
    sec2 = Security(security_uuid="s2", current_symbol="INFY", name="Infosys Ltd", listing_date=date(1995, 1, 1))
    master.register_security(sec1)
    master.register_security(sec2)

    membership.add_membership_record(MembershipRecord(security_uuid="s1", universe_id="NIFTY50", effective_from=date(2016, 1, 1)))
    membership.add_membership_record(MembershipRecord(security_uuid="s2", universe_id="NIFTY50", effective_from=date(2016, 1, 1)))
